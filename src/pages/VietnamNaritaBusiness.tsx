...
export default function VietnamNaritaBusiness() {
  ...
  const airlineCard = (
    <div style={{ flexShrink: 0, width: isMobile ? "100%" : "clamp(240px,28vw,320px)", background: "rgba(255,255,255,0.10)", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)", border: "1px solid rgba(255,255,255,0.22)", borderRadius: 20, overflow: "hidden", boxShadow: "0 8px 40px rgba(0,0,0,0.35)" }}>
    ...
    <div style={{ fontFamily: "'Inter', -apple-system, sans-serif", background: "#f5f5f3", minHeight: "100vh", color: "#1a1a1a" }}>
      ...
      <div style={{ position: "relative", height: "clamp(380px,52vw,600px)", overflow: "hidden" }}>
        ...
        <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "flex-end", padding: "clamp(24px,4vw,52px) clamp(20px,5vw,64px)", gap: "clamp(24px,4vw,60px)", flexWrap: "wrap" }}>
          ...
          {/* Desktop only — card sits inside the hero image */}
          {!isMobile && airlineCard}
        </div>
      </div>

      {/* Mobile only — card drops below hero, no clipping */}
      {isMobile && (
        <div style={{ background: "linear-gradient(180deg, #001e50 0%, #0a0a0a 100%)", padding: "20px 16px 24px" }}>
          {airlineCard}
        </div>
      )}
